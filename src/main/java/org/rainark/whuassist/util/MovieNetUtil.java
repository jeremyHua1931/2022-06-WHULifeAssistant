...
public class MovieNetUtil {
    ...
    private static final String INDEX_REQUEST_URL = "https://movie.douban.com/j/search_subjects?type=movie&tag=";
     ...
     */
    public static ArrayList<Movie> getMovies(int offset, int len, boolean resolveDetail, boolean init, String crawlTime) throws IOException, JSONException {
        URL request = new URL(INDEX_REQUEST_URL + "&" + PAGE_OFFSET + "=" + offset + "&" + PAGE_MAX + "=" + (offset + len));
        if (init) {
            request = new URL(INDEX_REQUEST_URL + "%E8%B1%86%E7%93%A3%E9%AB%98%E5%88%86&sort=rank" + "&" + PAGE_OFFSET + "=" + offset + "&" + PAGE_MAX + "=" + (offset + len));
        } else {
            request = new URL(INDEX_REQUEST_URL + "%E7%83%AD%E9%97%A8" + "&" + PAGE_OFFSET + "=" + offset + "&" + PAGE_MAX + "=" + (offset + len));
        }
