...
public class NovelNetUtil {
     ...
     * @param gender      ("" or "mm") if reader is a girl, the indexURL is  https://www.qidian.com/rank/mm
     * @param novelChoice (榜单类型) Select the type of novel ranking list, the indexURL is https://www.qidian.com/rank/榜单/
    ...
    public static ArrayList<Novel> getNovels(int len, String gender, String novelChoice, String time) throws IOException {
        ...
        if (!time.equals("")) {
            ...
            Date d = new Date();
            selectTime = spiltTime(DateFormat.getDateInstance().format(d));
        }
        ...
        for (int i = 0; i < (int) (len / onePageNovelNumber + 1); i++) {
            ...
            while (totalNumber < len && count < onePageNovelNumber) {
                ...
                String update = String.valueOf(tmp.getElementsByClass("update").text());
                ...
                URL novelURL = new URL("https:" + url1);
                URL image = new URL("https:" + url2);
                ...
                Novel novelTmp = new Novel(count + 1, name, author, novelURL, image, category, subcategory, completionStatus, update, introduction);
                result.add(novelTmp);
